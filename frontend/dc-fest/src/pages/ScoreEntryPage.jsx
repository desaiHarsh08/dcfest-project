...
const ScoreEntryPage = () => {
  ...
  const [scoreEntryFlag, setScoreEntryFlag] = useState(true);

  const { user } = useContext(AuthContext);

  useEffect(() => {
    if (user?.type == "REGISTRATION_DESK" || user?.type == "ATTENDANCE_DESK" || user?.type == "SCORE_SHEET_DESK") {
      navigate(-1);
    }
  }, [user, navigate]);
  ...
  return (
    <div className="scoring-department">
      ...
      <div className="d-flex justify-content-between">
        ...
        {selectedAvailableEvent && selectedRound && (
          <div>
            {/* <Button variant={scoreEntryFlag ? "info" : "light"} className={`${!scoreEntryFlag && "border"}`} onClick={() => setScoreEntryFlag(true)}>
