          ...
          {categoryItem?.availableEvents?.map((event, index) => (
            <Link to={`event/${event.slug}`} key={index} style={{ textDecoration: "none" }}>
              <Card className="event-card" style={{ minWidth: "350px", minHeight: "350px" }}>
                <Card.Img variant="top" src={`${import.meta.env.VITE_APP_NODE_ENV === "production" ? import.meta.env.VITE_APP_PREFIX : ""}/${event.slug}.jpg`} alt={event.title} className="img-fluid event-img" />
                <Card.Body>
